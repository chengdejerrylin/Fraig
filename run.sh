...
function fraig(){
   ...
   for i in ./tests.fraig/*.aag; do
      ...
      # run program
      ./fraig -f "./mydofile/"$name".do" | grep -v -e "Fraig:" -e "SAT:" -e "Total #FEC" -e "Sweeping" -e "Strashing" -e "Simplifying" > "myoutput/"$name".out"
      mv ./myoutput/ref.aag ./myoutput/my.aag
      ./ref/fraig -f "./mydofile/"$name".do" | grep -v -e "Fraig:" -e "Proving" -e "Total #FEC" -e "Updating" -e "Sweeping" -e "Strashing" -e "Simplifying" > "myoutput/"$name"-ref.out"
