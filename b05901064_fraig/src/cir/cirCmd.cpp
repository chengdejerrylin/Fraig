...
CirGateCmd::exec(const string& option)
{
   ...
   if (doFanin)
      cirMgr->reportFanin(level, thisGate);
   else if (doFanout)
      cirMgr->reportFanout(level, thisGate);
   else
      thisGate->reportGate();
